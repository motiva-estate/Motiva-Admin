...
function ClientEditor() {
  ...
  const { data: subsResult } = useQuery({
    ...
    queryFn: () => api.subscriptions.list({ clientId: id, limit: 100 }),
    enabled: !isNew,
  });
  ...
  const save = useMutation({
    mutationFn: async () => {
      const payload: Partial<Client> = {
        ...form,
        fullName:
          form.fullName?.trim() || [form.firstName, form.lastName].filter(Boolean).join(" ").trim(),
      };
  ...
  return (
    <div className="grid max-w-full grid-cols-1 md:gap-3 lg:grid-cols-4">
      ...
      <div className="space-y-2 lg:col-span-4 grid grid-cols-1 gap-3 lg:grid-cols-4">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div className="space-y-2">
                ...
                <Select
                  value={form.source}
                  onValueChange={(v) => setForm((f) => ({ ...f, source: v as ClientSource }))}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
        ...
        <Card className="lg:col-span-2">
          <CardHeader>
            <CardTitle>Projects subscribing to</CardTitle>
          </CardHeader>
          <CardContent>
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setNok({ firstName: e.target.value })}
                />
              </div>
              <div className="space-y-2">
                <Label>Last name</Label>
                <Input
                  ...
                  onChange={(e) => setNok({ lastName: e.target.value })}
                />
              </div>
              ...
              <CardContent className="space-y-3">
                {!subs || subs.length === 0 ? (
                  ...
                  subs.map((s) => {
                    ...
                    return (
                      <div key={s._id ?? s.id} className="rounded-md border border-border p-3">
                        <div className="flex items-start justify-between gap-2">
                          <div className="min-w-0">
                            <div className="truncate text-sm font-medium">{s.plan}</div>
                            <div className="mt-0.5 text-xs text-muted-foreground">
                              {s.projectRef
                                ? `${s.projectRefType ?? "project"} · ${s.projectRef}`
                                : "No project link"}
                            </div>
                          </div>
                        ...
                        <div className="mt-2 space-y-1">
                          ...
                          <div className="flex justify-between text-xs text-muted-foreground">
                            <span>
                              {s.currency} {paid.toLocaleString()} paid
                            </span>
                  ...
                  <Switch
                    ...
                    onCheckedChange={(v) =>
                      setForm((f) => ({
                        ...
                        notificationPrefs: {
                          email: v,
                          whatsapp: f.notificationPrefs?.whatsapp ?? false,
                        },
                      }))
                    }
                  />
                  ...
                  <Switch
                    ...
                    onCheckedChange={(v) =>
                      setForm((f) => ({
                        ...f,
                        notificationPrefs: {
                          email: f.notificationPrefs?.email ?? true,
                          whatsapp: v,
                        },
                      }))
                    }
