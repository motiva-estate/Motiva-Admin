...
function CompanyPage() {
  ...
  const save = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["company"] }); toast.success("Company info saved"); },
  });
  ...
  return (
    <div className="max-w-full">
      ...
      />
      {isLoading ? <FormSkeleton rows={6} /> : (
      <Card>
        <CardContent className="space-y-4 p-6">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              <Label>Founding year</Label>
              <Input type="number" value={form.foundingYear ?? ""} onChange={(e) => setForm({ ...form, foundingYear: Number(e.target.value) })} />
            </div>
          </div>
          <div className="space-y-2">
            <Label>Mission</Label>
          ...
          <div>
            <div className="mb-2 flex items-center justify-between">
              <Label>Stats</Label>
              <Button variant="outline" size="sm" onClick={addStat}><Plus className="mr-1 h-4 w-4" />Add stat</Button>
            </div>
            <div className="space-y-2">
              {stats.length === 0 && <p className="text-sm text-muted-foreground">No stats yet.</p>}
              {stats.map((s, i) => (
                <div key={i} className="grid grid-cols-[1fr_1fr_auto] gap-2">
                  <Input placeholder="Label (Homes delivered)" value={s.label} onChange={(e) => setStat(i, { label: e.target.value })} />
                  <Input placeholder="Value (120)" value={s.value} onChange={(e) => setStat(i, { value: e.target.value })} />
                  <Button variant="ghost" size="icon" className="text-destructive" onClick={() => removeStat(i)}>
