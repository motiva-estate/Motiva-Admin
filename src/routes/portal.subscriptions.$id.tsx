...
import { ArrowLeft, CalendarClock, CheckCircle2, Circle, ExternalLink, FileText, Lock, MessageSquare } from "lucide-react";
...
export const Route = createFileRoute("/portal/subscriptions/$id")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Full detail for this Motiva subscription — schedule, payments, documents and updates." },
...
function SubscriptionDetail() {
  ...
  const { data: subs } = useQuery({
    ...
    queryFn: async () => (await api.subscriptions.list()).filter((s) => s.clientId === user?.clientId),
  ...
  if (!sub) {
    return (
      <Card><CardContent className="py-12 text-center text-sm text-muted-foreground">
        Subscription not found. <Link to="/portal" className="ml-1 underline">Back to overview</Link>
      </CardContent></Card>
    );
  }
    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <Card>
          <CardContent className="space-y-3 py-5">
            <div className="text-[11px] uppercase tracking-wide text-muted-foreground">Next installment</div>
            {sub.nextDueDate && !fullyPaid ? (
              <>
                <div className="text-sm font-medium text-foreground">
                  {format(new Date(sub.nextDueDate), "PPPP")}
                </div>
                <Button size="sm" variant="secondary" disabled className="w-full" title="Live payments come in Phase 2">
              ...
              <div className="text-sm text-muted-foreground">Nothing scheduled — you're fully paid.</div>
            )}
          </CardContent>
        </Card>
      </div>
        ...
        <Card>
          <CardContent className="p-0">
            <ul className="divide-y divide-border">
              {schedule.map((r) => (
                <li key={r.index} className="flex items-center justify-between gap-3 px-4 py-3 text-sm">
                  <span className="flex items-center gap-3">
                    ...
                    <span>
                      ...
                      <span className="ml-2 text-xs text-muted-foreground">{format(new Date(r.dueDate), "PPP")}</span>
                    </span>
                  </span>
                  <span className="flex items-center gap-3">
                    <span className="text-sm">{currency(r.amount, sub.currency)}</span>
                    <Badge variant={r.status === "paid" ? "secondary" : r.status === "next" ? "default" : "outline"}>
                      {r.status}
                    </Badge>
                  </span>
                </li>
      ...
      <section>
        ...
        {payments && payments.length > 0 ? (
          ...
          <Card><CardContent className="py-6 text-center text-sm text-muted-foreground">No payments recorded yet.</CardContent></Card>
        )}
      </section>
      ...
      <section>
        ...
        {docs && docs.length > 0 ? (
                  ...
                  <CardContent className="flex items-start justify-between gap-3 py-4">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-muted-foreground">Uploaded {format(new Date(d.uploadedAt), "PP")}</div>
                        {!visible && (
                          <div className="mt-1 text-xs text-muted-foreground">
                            {d.visibility === "on_full_payment" ? "Unlocks once fully paid." : `Unlocks on: ${d.visibility.replace("on_milestone:", "")}`}
                          </div>
                        )}
                      </div>
                    </div>
          ...
          <Card><CardContent className="py-6 text-center text-sm text-muted-foreground">No documents attached yet.</CardContent></Card>
        )}
      </section>
      ...
      <section>
        ...
        {updates && updates.length > 0 ? (
            ...
            {updates.map((u) => (
              <Card key={u.id}>
                <CardContent className="space-y-3 py-4">
                  <div className="text-xs text-muted-foreground">{format(new Date(u.postedAt), "PPP")}</div>
          ...
          <Card><CardContent className="py-6 text-center text-sm text-muted-foreground">No updates yet.</CardContent></Card>
        )}
      </section>
    </div>
  );
